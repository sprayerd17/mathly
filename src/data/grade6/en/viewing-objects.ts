...
<div style="background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:10px;padding:14px 20px;margin-bottom:8px;">
  ...
  <p style="margin:0;color:#1e3a8a;font-size:15px;line-height:1.7;">If a composite object is <strong>symmetrical</strong> in every direction (for example, a smaller box centred exactly on top of a bigger square box), its front view and side view can be <strong>identical</strong>, even though the object clearly has depth as well as width. Always check for symmetry before assuming two views must look different.</p>
      ...
      workedExamples: [
        {
          question: 'Three storage boxes are placed in a row, touching each other. All three boxes have exactly the same width and depth, but their heights are different: the first is short, the second is tall, and the third is a medium height in between. Describe the front view, side view and top view.',
          steps: [
            '<strong>Front view</strong> — scanning left to right, the outline rises and falls to match each box\'s height: short, then a tall step up, then a step down to medium. This gives an irregular <strong>step shape</strong>, not a simple two-step shape.',
            '<strong>Side view</strong> — since every box has exactly the same width and depth, and they are lined up in a single row, looking from the side (end-on) you can only measure the tallest box\'s height — you get a <strong>plain rectangle</strong>, as tall as the tallest box.',
            '<strong>Top view</strong> — looking straight down, each box takes up the same footprint, so you see <strong>three equal squares in a row</strong> — the top view cannot show that the boxes are different heights.',
          ],
          answer:
  ...
  practiceSets: [
    ...
    // ═══════════════════════════════════════════════════════════════════════
    {
      ...
      questions: [
        ...
        { difficulty: 'Easy', question: 'Besides front, side and top, name the four other viewing positions used in Grade 6.', answer: 'Back, left and right (and below)', checkMode: 'self', explanation: 'Grade 6 uses six viewing positions in total: front, back, left, right, above (top) and below (bottom).' },
